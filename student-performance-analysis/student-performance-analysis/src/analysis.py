...
def crosstab_and_pivots(df):
    ...
    if "gender" in df.columns and "final_score" in df.columns:
        results["pivot_gender_mean_score"] = (df.pivot_table(index="gender", values="final_score", aggfunc="mean").round(3))
    if "region" in df.columns and "final_score" in df.columns:
        results["pivot_region_mean_score"] = (df.pivot_table(index="region", values="final_score", aggfunc="mean").round(3))
    return results
...
def hypothesis_study_hours(df):
    ...
    return {
        "n": int(len(x)),
        "pearson_correlation_coefficient": float(f"{r:.5f}"),
        "pearson_probability": float(f"{cor:.5f}"),
        "median_hours_split": float(f"{med:.2f}"),
        "mean_score_low_hours": float(f"{low.mean():.2f}"),
        "mean_score_high_hours": float(f"{high.mean():.2f}"),
        "test_statistics": float(f"{t_stat:.5f}"),
        "test_probability": float(f"{p:.5f}"),
        "interpretation": (f"Correlation(study hours related to scores) {corsig} (probability value={float(cor):.5f}). "
