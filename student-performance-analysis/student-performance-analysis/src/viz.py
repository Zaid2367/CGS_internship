...
def save_hist(df, col, figg, filename):
    ...
    plt.xlabel(col)
    plt.ylabel("Count")
    path = os.path.join(figg, filename)
    plt.tight_layout()
    plt.savefig(path, dpi=180)
    return path

def save_box(df, vc, gc, figg, filename):
...
def save_scatter_trend(df, x_col, y_col, figg, filename):
    ...
    plt.plot(xs, a * xs + b)
    plt.title(f"Scatter Trend {y_col} vs {x_col}")
    plt.xlabel(x_col)
    plt.ylabel(y_col)
    path = os.path.join(figg, filename)
    plt.tight_layout()
...
def save_grouped_bar_mean(df, gc, vc, figg, filename):
